...
export function DeviceStatusPanel({
  ...
  const onGasOptionsLoadedRef = useRef(null);

  useEffect(() => {
    onGasOptionsLoadedRef.current = onGasOptionsLoaded;
  }, [onGasOptionsLoaded]);
  ...
  useEffect(() => {
    async function fetchGases() {
      ...
      if (!deviceName) {
        setGasOptions([]);
        if (onGasOptionsLoadedRef.current) {
          onGasOptionsLoadedRef.current([]);
        }
        return;
      }
      ...
      try {
        ...
        if (onGasOptionsLoadedRef.current) {
          onGasOptionsLoadedRef.current(options);
        }
